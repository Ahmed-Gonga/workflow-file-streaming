...
struct nfs_inode {
	...
	/*
	 * sNFS per-file streaming state.
	 *
	 * snfs_enabled selects files that participate in the experimental
	 * streaming path. snfs_available_until is the highest byte offset that
	 * has been produced for this inode. Readers wait on snfs_wait_queue when
	 * they request data beyond that offset.
	 *
	 * This keeps normal NFS behaviour for non-streaming files and provides
	 * the hybrid execution model required by sNFS: some producer/consumer
	 * pairs stream partial data, while unrelated files continue as regular NFS.
	 */
	spinlock_t		snfs_lock;
	loff_t			snfs_available_until;
	wait_queue_head_t	snfs_wait_queue;
	bool			snfs_enabled;
